...
public class FeaturedTvShowsFragment extends Fragment {

    private static final String ARG_PARAM1 = "param1";
    private static final String ARG_PARAM2 = "param2";
    public static final String KEY = "tvShows_featured";
    ...
    public FeaturedTvShowsFragment() {
    ...
    public static FeaturedTvShowsFragment newInstance(String param1, String param2) {
        FeaturedTvShowsFragment fragment = new FeaturedTvShowsFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_featured_tvshows, container, false);
        return view;
    }
...
//        payMode = RabbitTvApplication.getInstance().getPaymode();
    ...
    private void loadContent(ArrayList<Slider> sliders, ArrayList<Carousel> carousels) {
        if (sliders == null && carousels == null)
            return;
        try {
            Fragment fragment = SlidersAndCarouselsFragment.newInstance(0, payMode, Constants.TV_SHOWS_FEATURED, sliders, carousels);
...
//        if (RabbitTvApplication.getInstance().getSliderBeanHashMapFeatured().containsKey(KEY) ||
//                RabbitTvApplication.getInstance().getHorizontalBeanHashMapFeatured().containsKey(KEY)) {
//            sliders = RabbitTvApplication.getInstance().getSliderBeanHashMapFeatured().get(KEY);
//            carousels = RabbitTvApplication.getInstance().getHorizontalBeanHashMapFeatured().get(KEY);
            ...
            Thread thread = new Thread() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        final JSONObject json = JSONRPCAPI.getOndemandSuggestions(mParam1); //updated on 22-02-2019
                       // final JSONObject json = JSONRPCAPI.getOndemandSuggestions("home");
                        if (json != null) {
                            handler.post(() -> {
                                ...
                                if (sliders.size() > 0)
                                    RabbitTvApplication.getInstance().getSliderBeanHashMapFeatured().put(KEY, sliders);
                                if (carousels.size() > 0)
                                    RabbitTvApplication.getInstance().getHorizontalBeanHashMapFeatured().put(KEY, carousels);
                                loadContent(sliders, carousels);
                            });
                        }
                    } catch (Exception e) {
